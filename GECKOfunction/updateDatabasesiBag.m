...
function [swissprot,kegg] = updateDatabasesiBag(keggID)
...
save('ProtDatabaseBag.mat','swissprot','kegg');
...
fileID_uni        = fopen('uniprotBag.tab');
...
for i = 1:min([numel(gene_id),10000])
    try
        ...
        fid  = fopen(['KEGG/' gene_id{i}{1} '.txt'],'w');
        fprintf(fid,'%s',gene);
        fclose(fid);
