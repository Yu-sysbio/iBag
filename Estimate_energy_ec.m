...
% model = changeRxnBounds(model,'arm_R0108',0,'b');
...
model = changeRxnBounds(model,'EX0001',-0.1,'l');
...
protein_cost_1 = sol_1.x(ismember(model.rxns,'prot_pool_exchange'))*10;
Yatp_1 = sol_1.x(ismember(model.rxns,'NGAM'))/-sol_1.x(ismember(model.rxns,'EX0001'));
prot_eff_1 = Yatp_1/protein_cost_1;

%% pathway 2: upper glycolysis + xpk + pdh + acetate fermentation + OXPHOS
% model_2 = model;
% model_2 = changeObjective(model_2,'EX0068');
% sol_2 = optimizeCbModel(model_2,'max','one');
% protein_cost_2 = sol_2.x(ismember(model.rxns,'prot_pool_exchange'))*10;
...
protein_cost_3 = sol_3.x(ismember(model.rxns,'prot_pool_exchange'))*10;
Yatp_3 = sol_3.x(ismember(model.rxns,'NGAM'))/-sol_3.x(ismember(model.rxns,'EX0001'));
prot_eff_3 = Yatp_3/protein_cost_3;

%% pathway 4: upper glycolysis + xpk + lactate fermentation
model_4 = model;
model_4 = changeRxnBounds(model_4,'arm_R0101_REV',0.15,'b');
model_4 = changeRxnBounds(model_4,'EX0068',0.15,'b');
model_4 = changeObjective(model_4,'EX0002');
sol_4 = optimizeCbModel(model_4,'max','one');
model_4 = changeRxnBounds(model_4,'EX0002',sol_4.f,'b');
model_4 = changeObjective(model_4,'prot_pool_exchange');
sol_4 = optimizeCbModel(model_4,'min','one');
protein_cost_4 = sol_4.x(ismember(model.rxns,'prot_pool_exchange'))*10;
...
% protein_cost_5 = sol_5.x(ismember(model.rxns,'prot_pool_exchange'))*10;
